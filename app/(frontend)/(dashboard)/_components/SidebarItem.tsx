...
const SidebarItem = ({ icon: Icon, label, href }: SidebarItemProps) => {
  ...
  return (
    <button onClick={onClick} type="button" className={cn(
      "flex items-center gap-x-2 text-slate-500 text-sm font-semibold pl-6 transition-all hover:text-slate-600 hover:bg-slate-300/20" , isActive && "text-sky-700 bg-sky-200/20 hover:text-sky-200/20 hover:text-sky-700"
    )}>
